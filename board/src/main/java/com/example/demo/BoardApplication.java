...
public class BoardApplication {
	...
	//       (프로그램의 시작을 담당하는 파일)
	//  -> src/main/resource (자바파일을 제외한 HTML, CSS, JS, 환경설정파일 저장영역)
	//       templates : html파일을 받는 영역.
	//        -> 템플릿 : 자바코드를 받을수 있는 HTML형식의 파일
	...
	//       테스트 영역
	//  -> 그 외
	//  		bulid.gradle : 프로젝트에 필요한 플러그인, 라이브러리를 설치하는 영역
	...
	//   -> JPA를 구현한 실제 클래스중 Hibernate 라는것이 있음.
	...
	// @Column : 열의 세부설정
	//    -> length : 길이 설정
	//    -> columnDefinition = "TEXT" : 텍스트(문자열)을 데이터로 받을때.
	
	// @OneToMany
	//  1개의 엔티티에 여러개의 맴버변수 참조가 가능하도록 설정
	
	// @ManyToOne
	// 여러개의 맴버변수참조를 한개의 클래스(엔티티)로 하도록 설정
...
// 		mybatis vs JPA
// 		Model
// 		mybatis : 데이터베이스 SQL(쿼리) 자체를 직접입력하여 DB에 전송하는 방식
